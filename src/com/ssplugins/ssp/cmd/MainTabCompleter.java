...
public class MainTabCompleter implements TabCompleter {
		...
		if (index == 1) return getSubcommands(sender);
			...
			switch (args[0].toLowerCase()) {
				...
				case "g":
					...
					return Stream.concat(SSPermissions.getAPI().getGroupManager().getGroups().stream().map(Group::getName), Stream.of("default")).collect(Collectors.toList());
				case "player":
				case "p":
			...
			switch (args[2].toLowerCase()) {
				...
				case "move":
				case "m":
					return SSPermissions.getAPI().getGroupManager().getGroups().stream().map(Group::getName).collect(Collectors.toList());
			}
		}
	...
	private List<String> getPermissions(String name, boolean group) {
		if (group) {
			Optional<Group> op = SSPermissions.getAPI().getGroupManager().getGroup(name);
			if (op.isPresent()) return new ArrayList<>(op.get().getPermissions().getAll());
		}
		else {
			Optional<SSPlayer> op = SSPermissions.getAPI().getPlayerManager().getPlayerByName(name);
			if (op.isPresent()) return new ArrayList<>(op.get().getPermissions().getAll());
		}
		return new ArrayList<>();
	}
